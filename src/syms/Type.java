...
public abstract class Type 
{
    ...
    protected Type( String name, int n, boolean resolved ) {
        this( n, resolved );
        this.name = name;
    }
     ...
     */
    public static final Type ERROR_TYPE = new Type( "error_type", 0, true ) {
    ...
    public ArrayType getArrayType() {
        ...
        if( thisType instanceof ArrayType ) {
            return (ArrayType)thisType;
        }
        return null;
    }
    /** Type representing an array. */
    public static class ArrayType extends FunctionType {
        
        public ArrayType( Type indexType, Type elementType ) {
            super( indexType, elementType );           // Type size will need to be resolved later
            this.resolved = false;
        }
        @Override
        public ArrayType resolveType( Position pos ) {
            super.resolveType( pos );
            /* Calculate the size of the whole array */
            if( argType instanceof SubrangeType 
            ) {
                ScalarType range = (ScalarType)argType;
                space = resultType.getSpace() * 
                        (1 + range.getUpper() - range.getLower());
            } else if( argType != ERROR_TYPE ) {
                errors.error( "index type of an array must be a subrange", 
                        pos );
            }
            return this;
        }
        ...
        @Override
        public String toString() {
            return "ARRAY " + argType + " OF " + resultType;    
        }
    }
}
